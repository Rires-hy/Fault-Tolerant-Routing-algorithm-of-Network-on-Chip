...
// modified by Kiyan
vector<tuple<int,int,int> > ReservationTable::getReservations2(const int port_in)
{
    vector<tuple<int,int,int>> reservations;
	...
	if (rtable[o].reservations.size()>0)
	{
	    int current_index = rtable[o].index;
	    if (rtable[o].reservations[current_index].input == port_in)
		reservations.push_back(tuple<int,int,int>(o, rtable[o].reservations[current_index].vc, rtable[o].reservations[current_index].new_vc));
	}
    }
    return reservations;
}
