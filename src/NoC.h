...
SC_MODULE(NoC)
{
    ...
    std::map<int,int> PermutationTable;
	...
	if ((GlobalParams::topology == TOPOLOGY_MESH) && (GlobalParams::my_topology == 0)){
	    // Build the Mesh
	    buildMesh();
        std::cout << "The value of my_topology is " << GlobalParams::my_topology << endl;
    }
	    ...
	    buildOmega();
	else if ((GlobalParams::topology == TOPOLOGY_MESH) && (GlobalParams::my_topology == 1))
        buildMyTopology();
    else {
