...
void NoC::buildMesh()
{
	...
	// temperary change: add one broken wire here
	// future design: add broken wire to specification files (Kiyan Nan)
	tuple<int,int> brokenID(dimX/2, dimY/2);
	Coord broken_id;
	broken_id.x = get<0>(brokenID);
	broken_id.y = get<0>(brokenID);
	int broken_tile_id = coord2Id(broken_id);
	...
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
	    ...
	    t[i][j] = new Tile(tile_name, tile_id);
		// if broken, set broken link (KIyan Nan)
		if (tile_id == broken_tile_id){
			t[i][j]->r->broken_direction[DIRECTION_SOUTH] = true;
			// t[i][j]->r->broken_direction[DIRECTION_EAST] = true;
			cout <<  "\n\n\nTile " << tile_coord.x << " " << tile_coord.y << " South and East Broken!\n\n" << endl;
		} else if (broken_id.x == tile_coord.x && broken_id.y == (tile_coord.y-1)){
			// t[i][j]->r->broken_direction[DIRECTION_NORTH] = true;
			cout <<  "\n\n\nTile " << tile_coord.x << " " << tile_coord.y << " North Broken!\n\n" << endl;
		} else if (broken_id.x == (tile_coord.x-1) && broken_id.y == tile_coord.y){
			t[i][j]->r->broken_direction[DIRECTION_WEST] = true;
			cout <<  "\n\n\nTile " << tile_coord.x << " " << tile_coord.y << " West Broken!\n\n" << endl;
		}
...
void NoC::asciiMonitor()
{
	...
	// UPDATE GLOBAL PERMUTATION TABLE
	if ((GlobalParams::topology == TOPOLOGY_MESH) && (GlobalParams::my_topology == 1)){
		int currentCycle = sc_time_stamp().to_double() / GlobalParams::clock_period_ps - GlobalParams::reset_time;
		if ((currentCycle%PERMUTATION_CYCLE)==0){
			std::mt19937 g(static_cast<uint32_t>(time(0)));
			Randomindex = Nodeindex;
			std::shuffle ( Randomindex.begin(), Randomindex.end(), g);
			for(int kc=0;kc<(int)Nodeindex.size();kc++){
        		PermutationTable[Nodeindex[kc]] = Randomindex[kc];
    		}
		}
	}
...
void NoC::buildMyTopology()
{
	globalGraph.build(1); // 0 for dfs ;otherwise bfs
	// initialize index
	for(int idaa=0;idaa<GlobalParams::mesh_dim_x*GlobalParams::mesh_dim_y;idaa++){
		Coord dirID = id2Coord(idaa);
    	int ka = dirID.x;
    	int kb = dirID.y;
		if (!isBroken(ka,kb)){
			Nodeindex.push_back(idaa);
		}
	}
	// permutation first
	std::mt19937 g(static_cast<uint32_t>(time(0)));
    ...
    // for(auto num: Nodeindex){
    //     std::cout << "Index: " << num << ". The destination:  " <<  PermutationTable[num] << std::endl;;
    // }

    buildCommon();
	cout << "Running my topology now. The value of my_topology is " << GlobalParams::my_topology << endl;
	...
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
	    ...
	    t[i][j]->pe->local_id = j * GlobalParams::mesh_dim_x + i;
		// assign global graph
		t[i][j]->r->globalGraph= &globalGraph;
		t[i][j]->pe->PermutationTable = &PermutationTable;
	    // Check for traffic table availability
   		if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED)
		{
			 t[i][j]->pe->traffic_table = &gttable;	// Needed to choose destination
	...
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
		for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {		
			if(isBroken(i,j)){
				t[i][j]->r->broken_routers=true;
				for(int dir=0;dir<DIRECTIONS; dir++){
					t[i][j]->r->broken_direction[i]=true;
				}
				t[i][j]->pe->never_transmit=true;

				//remove 4 direction of itself
				// remove north
				req[i][j].south = 0;
				ack[i][j].north = 0;
				req[i][j].north = 0;
				...
				//remove north tile connection:
				if (j>0){
					t[i][j-1]->r->broken_direction[DIRECTION_SOUTH]=true;
					free_slots[i][j-1].north.write(NOT_VALID);
					nop_data[i][j-1].north.write(tmp_NoP);
				}
				//remove west tile connection:
				if (i>0){
					t[i-1][j]->r->broken_direction[DIRECTION_EAST]=true;
					free_slots[i-1][j].west.write(NOT_VALID);
					nop_data[i-1][j].west.write(tmp_NoP);
				}
				//remove east tile connection:
				if (i!=15){
					t[i+1][j]->r->broken_direction[DIRECTION_WEST]=true;
					free_slots[i+1][j].east.write(NOT_VALID);
					nop_data[i+1][j].east.write(tmp_NoP);
				}
				//remove SOUTH tile connection:
				if (j!=15){
					t[i][j+1]->r->broken_direction[DIRECTION_NORTH]=true;
					free_slots[i][j+1].south.write(NOT_VALID);
					nop_data[i][j+1].south.write(tmp_NoP);
				}
			}
		}
	}
