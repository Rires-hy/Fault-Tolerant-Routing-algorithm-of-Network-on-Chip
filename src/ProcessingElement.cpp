...
bool ProcessingElement::canShot(Packet & packet)
{
   // assert(false);
    ...
    if (currentCycle > GlobalParams::stop_cycle)
        return false;
	...
	if (shot) {
        ...
        else if (GlobalParams::traffic_distribution == TRAFFIC_PERMUTATION)
		    packet = trafficPermutation();
        else {
...
Packet ProcessingElement::trafficRandom()
{
    ...
    Coord dirID = id2Coord(p.dst_id);
    i = dirID.x;
    j = dirID.y;

	// check for hotspot destination
	for (size_t kk = 0; kk < GlobalParams::hotspots.size(); kk++) {

	    if (rnd >= range_start && rnd < range_start + GlobalParams::hotspots[kk].second) {
		if (local_id != GlobalParams::hotspots[kk].first ) {
		    p.dst_id = GlobalParams::hotspots[kk].first;
		}
		break;
	    } else
		range_start += GlobalParams::hotspots[kk].second;	// try next
	}
#ifdef DEADLOCK_AVOIDANCE
	assert((GlobalParams::topology == TOPOLOGY_MESH));
	if (p.dst_id%2!=0)
    ...
    } while ((p.dst_id == p.src_id) || isBroken(i,j));

    p.timestamp = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
    p.size = p.flit_left = getRandomSize();
    p.vc_id = randInt(0,GlobalParams::n_virtual_channels-1);
    // if (local_id ==32){
    //     cout << "Package generated from: " << local_id << " to " << p.dst_id << endl;
    // }
...
Packet ProcessingElement::trafficBitReversal()
{
    ...
    Coord dirID = id2Coord(dnode);
    int i = dirID.x;
    int j = dirID.y;

    while ((dnode == local_id) || isBroken(i,j)){
        if (dnode >= 0 && dnode < (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)/4){
            dnode++;
        } else if (dnode >= (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)/4 && dnode < (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)/2){
            dnode--;
        } else if (dnode >= (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)/2 && dnode < 3*(GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)/4){
            dnode++;
        } else {
            dnode--;
        }
        dirID = id2Coord(dnode);
        i = dirID.x;
        j = dirID.y;
    }
...
Packet ProcessingElement::trafficPermutation(){
    Packet p;
    p.src_id = local_id;
    p.dst_id = (*PermutationTable)[local_id];
    p.vc_id = randInt(0,GlobalParams::n_virtual_channels-1);
    p.timestamp = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
    p.size = p.flit_left = getRandomSize();
    return p;
