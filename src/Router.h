...
SC_MODULE(Router)
{
    ...
    bool broken_direction[DIRECTIONS]={false,false,false,false}; // The broken direction Flag (Kiyan Nan)
    bool broken_routers=false;
    ...
    SC_CTOR(Router) {
        ...
        // else {
        //   cout << "The routing algorithm is: " << GlobalParams::routing_algorithm << endl;
        // }
  ...
  private:
    ...
    int route(RouteData& route_data);

    // wrappers
    int selectionFunction(const vector <int> &directions,
			  RouteData& route_data);
    vector < int >routingFunction(RouteData& route_data);
